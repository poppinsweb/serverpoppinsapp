...
const saveCompleteEvaluation = async (
  evaluationtoken,
  evaluationId,
  responses
) => {
  try {
    const evaluationToken = await EvaluationToken.findOne({
    ...
    if (evaluationToken.usageCount >= 2) {
      throw new Error(
        "Token has been used the maximum number of times allowed"
      );
    }

    const completeEvaluation = await CompleteEvaluation.findOne({
      evaluationtoken,
      evaluationId,
    });
